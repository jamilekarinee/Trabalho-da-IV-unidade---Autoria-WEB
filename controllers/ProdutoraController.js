...
router.post('/', (req, res) => {
  const { nome } = req.body;
  executarConsulta('INSERT INTO produtora (nome) VALUES (?)', [nome], res, "Erro no cadastro de produtora!");
});
...
router.put('/', (req, res) => {
  ...
  executarConsulta('UPDATE produtora SET nome = ? WHERE id = ?', [nome, id], res, "Erro ao atualizar produtora");
});
